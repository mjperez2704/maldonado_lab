...
export default function CultivosTable({ initialCultivos }: { initialCultivos: Culture[] }) {
  const [cultivos, setCultivos] = useState<Culture[]>(initialCultivos);
  ...
  const handleDelete = async (id: string) => {
      if (confirm('¿Estás seguro de que quieres eliminar este cultivo?')) {
          try {
              ...
              setCultivos(cultivos.filter(c => c.id !== Number(id)));
              toast({ title: "Éxito", description: "Cultivo eliminado." });
          } catch (error) {
              console.error("Error deleting culture: ", error);
              toast({ title: "Error", description: "No se pudo eliminar el cultivo.", variant: "destructive" });
          }
      }
  }

  const handleEdit = (id: string) => {
    router.push(`/cultivos/editar/${id}`);
  }
      ...
      <div className="overflow-x-auto">
          <Table>
              ...
              {cultivos.map((cultivo, index) => (
                  ...
                  <TableCell>
                      <div className="flex items-center gap-2">
                      <Button variant="outline" size="icon" onClick={() => handleEdit(String(cultivo.id))}>
                          <Pencil className="h-4 w-4" />
                      </Button>
                      <Button variant="destructive" size="icon" onClick={() => handleDelete(String(cultivo.id))}>
       ...
       <div className="flex justify-between items-center">
          <div className="text-sm text-muted-foreground">
              Mostrando 1 a {cultivos.length} de {cultivos.length} registros
          </div>
          <Pagination>
              <PaginationContent>
                  <PaginationItem>
