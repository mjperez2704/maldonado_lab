...
export default function EditCulturePage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await updateCulture(cultureId, formData);
            router.push('/cultivos');
        } catch (error) {
    ...
    <div className="flex flex-col gap-4 py-8">
      <div className="flex justify-between items-center">
        ...
        <div className="text-sm text-muted-foreground">
            <Link href="/" className="hover:text-primary">Hogar</Link> / <Link href="/cultivos" className="hover:text-primary">Cultivos</Link> / Editar cultivo
        </div>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Editar cultivo</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col gap-8">
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                ...
                <div className="flex justify-start">
                    <Button type="submit">
                        <Check className="mr-2"/> Guardar Cambios
                    </Button>
                </div>
            </form>
        </CardContent>
