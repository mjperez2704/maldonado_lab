...
// Estado inicial alineado con la nueva interfaz StudyDetails
const initialFormData: Omit<StudyDetails, 'id'> = {
    name: '',
    code: '',
    type: 'ESTUDIO',
    price: 0,
...
export default function CreateStudyPage() {
    ...
    const [formData, setFormData] = useState<Omit<StudyDetails, 'id'>>(initialFormData);
    const [categories, setCategories] = useState<Category[]>([]);
    const [allStudies, setAllStudies] = useState<Service[]>([]); // Usar la interfaz Service simplificada
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [cats, studies] = await Promise.all([getCategories(), getAllStudies()]);
                setCategories(cats);
                setAllStudies(studies);
            } catch (error) {
                console.error("Error fetching dependencies", error);
                toast({ title: "Error", description: "No se pudieron cargar las dependencias.", variant: "destructive" });
            }
        };
        fetchData();
    }, [toast]);
    ...
    const handleSelectChange = (id: keyof Omit<StudyDetails, 'id'>, value: string | number | null) => {
    ...
    const handleAddParameter = () => {
        ...
        setFormData(prev => ({ ...prev, parameters: [...prev.parameters, { ...newParam, order: newOrder }] }));
        setNewParam({ name: '', unit: '', reference_value: '', order: 0 }); // Reset
    };
    ...
    const handleAddSample = () => {
        ...
        setNewSample({ type: '', container: '', indications: '' }); // Reset
    };
    ...
    const addPackageItem = (study: Service) => {
        ...
        setFormData(prev => ({ ...prev, packageItems: [...(prev.packageItems || []), newItem] }));
        setStudySearchTerm('');
    };

    const removePackageItem = (itemId: number) => {
        setFormData(prev => ({ ...prev, packageItems: (prev.packageItems || []).filter(pi => pi.item_id !== itemId) }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast({ title: "Error", description: "No se pudo crear el estudio. Revise la consola para más detalles.", variant: "destructive" });
        } finally {
            setLoading(false);
        }
    };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 py-8">
            ...
            <div className="flex justify-end">
                <Button type="submit" size="lg" disabled={loading}>
                    {loading ? 'Guardando...' : <><Save className="mr-2 h-4 w-4" /> Guardar Estudio</>}
                </Button>
            </div>
        </form>
    );
