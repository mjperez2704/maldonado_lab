...
export default function AntibioticsTable({ initialAntibiotics }: { initialAntibiotics: Antibiotic[] }) {
  const [antibiotics, setAntibiotics] = useState<Antibiotic[]>(initialAntibiotics);
  const router = useRouter();
  const { toast } = useToast();
  const loader = useLoader();

  const handleDelete = async (id: number) => {
      if (confirm('¿Estás seguro de que quieres eliminar este antibiótico?')) {
          loader.start('delete');
          try {
              await deleteAntibiotic(id);
              setAntibiotics(antibiotics.filter(a => a.id !== id));
              toast({ title: "Éxito", description: "Antibiótico eliminado." });
          } catch (error) {
              console.error("Error deleting antibiotic: ", error);
              toast({ title: "Error", description: "No se pudo eliminar el antibiótico.", variant: "destructive" });
          } finally {
              loader.stop();
          }
      }
  ...
  const handleEdit = (id: number) => {
    router.push(`/antibioticos/editar/${id}`);
  }
      ...
      <div className="overflow-x-auto">
          <Table>
              ...
              <TableRow>
                  ...
                  <TableHead>
                       <Button variant="ghost" size="sm">Atajo <ArrowUpDown className="ml-2 h-4 w-4" /></Button>
                  </TableHead>
       ...
       <div className="flex justify-between items-center">
          <div className="text-sm text-muted-foreground">
              Mostrando 1 a {antibiotics.length} de {antibiotics.length} registros
          </div>
          <Pagination>
              <PaginationContent>
                  <PaginationItem>
